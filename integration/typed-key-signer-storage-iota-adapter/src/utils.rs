    ...
    let public_key = match key_type {
        KeyType::Ed25519DerEncoded => {
            let public_key_bytes =
                ...
                .map_err(|e| SecretStorageError::Other(anyhow!("failed to decode ED25519 public key; {e}")))?;

            IotaKeySignaturePublicKey::try_from_bytes(
                IotaSignatureScheme::ED25519,
                &public_key_bytes.to_bytes(),
            )
            .map_err(|e| SecretStorageError::Other(anyhow!("failed to create IOTA public key from bytes; {e}")))?
        }
        KeyType::Secp256r1DerEncoded => {
            let decoded = p256::PublicKey::from_public_key_der(public_key_der)
                .map_err(|e| SecretStorageError::Other(anyhow!("failed to decode SECP256R1 public key; {e}")))?;
        ...
        KeyType::Secp256k1DerEncoded => {
            let decoded = k256::PublicKey::from_public_key_der(public_key_der)
                .map_err(|e| SecretStorageError::Other(anyhow!("failed to decode SECP256K1 public key; {e}")))?;
            let sec1_bytes = decoded.to_sec1_bytes();
