...
#[wasm_bindgen(js_name = IotaCompatibleJwkStorage)]
pub struct WasmIotaCompatibleJwkStorage(
    IotaCompatibleJwkStorage<WasmKeyStorageTypedKeySignature>,
);

/// Implementation to support `JwkStorage` behavior.
#[wasm_bindgen(js_class = IotaCompatibleJwkStorage)]
impl WasmIotaCompatibleJwkStorage {
    #[wasm_bindgen(constructor)]
    pub fn new(storage: WasmKeyStorageTypedKeySignature) -> Self {
        Self(IotaCompatibleJwkStorage::new(storage))
    }
    ...
    pub async fn public_key_jwk(&self, key_id: String) -> Result<WasmPublicJwk, JsValue> {
        let jwk = self.0.public_key_jwk(&key_id)
            .await
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub async fn insert(&self, _jwk: JsValue) -> Result<String, JsValue> {
        Err(JsValue::from_str(
            "Insert operation not supported for IotaCompatibleJwkStorage.",
        ))
    }
