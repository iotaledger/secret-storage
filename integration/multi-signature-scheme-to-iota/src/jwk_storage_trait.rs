...
where
    TInner: KeyGenerate<SignatureSchemeMulti, String, Options: TryFrom<MultiSignatureSchemeKeyType>>
        + KeySignWithOptions<SignatureSchemeMulti, String, Options: TryFrom<MultiSignatureSchemeKeyType>>
        + KeyDelete<String>
        + KeyExist<String>
        + OptionalSync
        + OptionalSend,
    <TInner as KeySignWithOptions<SignatureSchemeMulti, String>>::Signer: OptionalSend,
{
    async fn generate(
        ...
        let key_type = identity_key_type_to_multi_signature_scheme(&key_type)?;
        let signature_type_from_alg: MultiSignatureSchemeKeyType = alg_to_key_type(&alg)?;
    ...
    async fn sign(
        ...
        let key_type: MultiSignatureSchemeKeyType = alg_to_key_type(&alg)?;
...
fn identity_key_type_to_multi_signature_scheme(
    key_type: &IdentityStorageKeyType,
