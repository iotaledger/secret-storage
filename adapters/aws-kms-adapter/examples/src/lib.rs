  ...
  SK: secret_storage::KeyGet<IotaKeySignature, String> + OptionalSync,
{
  ...
  let method: VerificationMethod = VerificationMethod::new_from_jwk(unpublished.id().clone(), jwk, None).unwrap();
  ...
  // operation fails.
  <I as KeyIdStorage>::insert_key_id(storage.key_id_storage(), method_digest, KeyId::new(key_id))
    .await
    .unwrap();
...
pub async fn get_read_only_client() -> anyhow::Result<IdentityClientReadOnly> {
  ...
  let package_id = std::env::var("IOTA_IDENTITY_PKG_ID")
    .map_err(|e| anyhow!("env variable IOTA_IDENTITY_PKG_ID must be set in order to run the examples").context(e))
  ...
  I: KeyIdStorage,
{
  let package_id = std::env::var("IOTA_NOTARIZATION_PKG_ID")
    .map_err(|e| anyhow!("env variable IOTA_NOTARIZATION_PKG_ID must be set in order to run this example").context(e))
    .and_then(|pkg_str| pkg_str.parse().context("invalid package id"))?;
...
pub async fn run_example_for_key_config(key_config: &KeyConfig) -> Result<(), anyhow::Error> {
  ...
  let (identity_client, document, vm_fragment, keys_to_delete, tx_key_id) = if let Some(key_id) = &key_config.key_id {
    ...
    let tx_key = storage
      .key_storage()
      .generate(
        IdentityStorageKeyType::from_static_str(key_spec_to_identity_key_type(&key_config.tx_sign_key_spec)),
    ...
    let jwk_key = storage
      .key_storage()
      .generate(
        IdentityStorageKeyType::from_static_str(key_spec_to_identity_key_type(&key_config.jwk_sign_key_spec)),
        key_config.jws_algorithm,
      )
      .await?;
  ...
  let decoded_credential: DecodedJwtCredential<Object> = match key_config.jws_algorithm {
    JwsAlgorithm::EdDSA => JwtCredentialValidator::with_signature_verifier(EdDSAJwsVerifier::default())
      .validate::<_, Object>(
        &credential_jwt,
        &document,
  ...
  if let Some(id) = tx_key_id {
    println!(
      "Tx signing key kept — reuse with: AWS_KEY_ID={id} in 1_use_existing_key or schedule for deletion with: 2_schedule_key_deletion"
    );
  }
